...
class Program
{
    ...
    static string[] FilterArray(string[] inputArray)
    {
        ...
        // Обратить внимание на использование string?
        // для разрешения присваивания null
        int currentIndex = 0;
        foreach (var item in inputArray)
        {
            if (item != null && item.Length <= 3)
        ...
        return resultArray.Select(x => x ?? "").ToArray();
        // Возвращаемый массив не должен содержать null, поэтому преобразуем
        // его в массив строк и заменяем null на пустую строку
    }
}